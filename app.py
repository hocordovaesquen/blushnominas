...
def procesar_datos(uploaded_file):
    try:
        ...
        # Leemos las primeras 30 filas sin cabecera para buscar dónde están los títulos reales
        ...
        for idx, row in df_preview.iterrows():
            ...
            # Buscamos una fila que tenga 'EMPLEADO' y alguna columna de monto
        ...
        # 3. LIMPIEZA DE NOMBRES DE COLUMNAS (SANEAMIENTO)
        ...
        # 4. SELECCIÓN INTELIGENTE DE COLUMNA DE MONTO (SOLUCIÓN AL ERROR)
        col_monto = None
        
        # Prioridad 1: Columna 'TOTAL' (Suele ser el total del item/línea)
        if 'TOTAL' in df.columns:
             # Verificamos si es un DataFrame (duplicados) o Series
             if isinstance(df['TOTAL'], pd.DataFrame):
                 # Si hay dos columnas TOTAL, tomamos la última (asumiendo que es la calculada final)
        ...
        # Prioridad 2: Si no hay TOTAL válido, usamos TOTAL COMP
...
def crear_excel_con_formulas(df, resumen_df):
    ...
    # Métricas Grandes (Jalan el total de la hoja NOMINA)
    ws2['B4'] = "VENTA TOTAL"
    ...
    for r, row in enumerate(df.itertuples(), 2):
        ...
        ws3.cell(r, 3, row._4) # Producto/Servicio (Col 4 del dataframe original)
...
# --- INTERFAZ DE USUARIO ---
st.markdown('<div class="main-header">💇‍♀️ BLUSH SYSTEM v5.0 (Final)</div>', unsafe_allow_html=True)
