...
# --- CONSTANTES ---
# Por ahora no usamos una lista fija de palabras clave.
# La detección de PRODUCTO se basa en:
#   - Columna CLASE = "Producto", o
#   - Que el texto del ITEM tenga "ML" (ej. x250ml, 300 ML, 500ml, etc.)
...
def procesar_datos(uploaded_file):
    try:
        ...
        sheet_name = next(
            (s for s in xl.sheet_names if 'ventas' in s.lower() or 'hoja1' in s.lower()),
            xl.sheet_names[0]
        )
        ...
        # Regla:
        #  - Producto si CLASE = "PRODUCTO"
        #  - o si el texto del item contiene "ML" (ej. x250ml, 300 ML)
        cond_clase = df['CLASE_UPPER'] == 'PRODUCTO'
        cond_ml = df['ITEM_UPPER'].str.contains('ML', na=False)
        df['ES_PRODUCTO'] = cond_clase | cond_ml
        
        # Comisiones
        c_producto = df['ES_PRODUCTO']
        c_julio_serv = (df['EMPLEADO'] == 'Julio') & (~df['ES_PRODUCTO'])
        c_jy_corte = (
            df['EMPLEADO'].isin(['Jhon', 'Yuri'])
            & (~df['ES_PRODUCTO'])
...
def generar_resumen(df):
    ...
    resumen['PARTICIPACION'] = (
        resumen['TOTAL_PRODUCCION'] / resumen['TOTAL_PRODUCCION'].sum()
    )
...
def crear_excel_con_formulas(df, resumen_df):
    ...
    headers = [
        '#', 'EMPLEADO', 'PROD. SERVICIOS', 'COM. SERVICIOS',
        'PROD. PRODUCTOS', 'COM. PRODUCTOS', 'TOTAL PROD.',
        'TOTAL COM.', 'DESCUENTOS', 'EXTRAS', 'A PAGAR'
    ]
    ...
    for idx, row in df.iterrows():
        ...
        ws3.cell(r, 3, row['PRODUCTO / SERVICIO'])
...
# --- UI ---
st.markdown('<div class="main-header">💇‍♀️ BLUSH SYSTEM v6.0 (Stable)</div>', unsafe_allow_html=True)
