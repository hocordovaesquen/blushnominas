...
# --- CONSTANTES ---
# (No se necesitan patrones de detección - se usa solo la columna CLASE del Excel)
...
def procesar_datos(uploaded_file):
    try:
        ...
        # Detección Productos - LÓGICA ESTRICTA
        # REGLA: Solo es producto si CLASE = "Producto" explícitamente
        # Todo lo demás (CLASE = "Servicio" o CLASE = NaN) → es SERVICIO
        
        df['CLASE_UPPER'] = df['CLASE'].astype(str).str.upper().str.strip()
        df['ES_PRODUCTO'] = df['CLASE_UPPER'] == 'PRODUCTO'
        
        # Para detalle, necesitamos ITEM_UPPER para las reglas de comisión
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        with tab1:
            ...
            # --- CORRECCIÓN DEL ERROR DE LOG: Eliminado background_gradient ---
            st.dataframe(
