...
def calcular_comision(empleado, servicio, monto, es_producto):
    empleado = str(empleado).strip()
    
    if es_producto:
        return monto * 0.10, 0.10, "Producto 10%"
    
    if empleado in ['Jhon', 'Yuri']:
        if pd.notna(servicio) and 'corte' in str(servicio).lower():
            return monto * 0.35, 0.35, "Corte 35%"
        else:
            return monto * 0.25, 0.25, "Servicio 25%"
    elif empleado == 'Julio':
        return monto * 0.40, 0.40, "Servicio 40%"
    else:
        return monto * 0.25, 0.25, "Servicio 25%"
...
def es_producto(nombre_item, clase):
    ...
    Primero chequea la columna CLASE, si es NaN usa el nombre
    """
    # Si CLASE tiene valor, usarlo
    if pd.notna(clase):
        return str(clase).upper().strip() == 'PRODUCTO'
    
    # Si CLASE es NaN, detectar por el nombre
    if pd.isna(nombre_item):
        return False
    
    nombre_upper = str(nombre_item).upper()
    
    # Palabras clave que indican PRODUCTO
    palabras_producto = [
...
def procesar_datos(uploaded_file):
    ...
    # DETECCION CORRECTA DE PRODUCTO VS SERVICIO
    df['ES_PRODUCTO'] = df.apply(
        lambda row: es_producto(row['PRODUCTO / SERVICIO'], row['CLASE']),
        axis=1
    )
    ...
    df['COMISION'] = resultados.apply(lambda x: x[0])
    df['PORCENTAJE'] = resultados.apply(lambda x: x[1])
    df['TIPO_COMISION'] = resultados.apply(lambda x: x[2])
...
if uploaded_file:
    try:
        ...
        promedio_com = total_com / num_empleados
        ...
        with tab3:
            ...
            display_detalle = df_filtrado[[
                ...
                'MONTO', 'TIPO_COMISION', 'COMISION'
            ]].copy()
            
            display_detalle['TIPO_REAL'] = df_filtrado['ES_PRODUCTO'].apply(lambda x: 'Producto' if x else 'Servicio')
            
            display_detalle.columns = ['Fecha', 'Empleado', 'Servicio/Producto', 
                                       'Monto', 'Regla', 'Comisión', 'Tipo']
...
<div style='text-align: center; color: #666;'>
    ...
    <p style='font-size: 0.8rem;'>Sistema de Cálculo de Comisiones v1.1</p>
</div>
""", unsafe_allow_html=True)
